...
class AdesaoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for fname in ['ano_trimestre', 'periodo_apuracao', 'codigo_receita', 'saldo', 'credito_original_utilizado', 'codigo_receita_denominacao', 'periodo_apuracao_um']:
    ...
    def clean(self):
        ...
        if metodo == 'Pedido de compensação':  # Ressarcimento
            ...
            if cleaned.get('saldo') in (None, ''):
                self.add_error('saldo', 'Informe o saldo.')
            ...
            # Exigir: periodo_apuracao, codigo_receita, saldo, codigo_receita_denominacao, periodo_apuracao_um
            if not cleaned.get('periodo_apuracao'):
                self.add_error('periodo_apuracao', 'Informe o Período de Apuração.')
            if not cleaned.get('codigo_receita'):
                self.add_error('codigo_receita', 'Informe o Código da Receita.')
